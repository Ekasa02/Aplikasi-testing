<template>
    <div class="bg-[#33333385] fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center ">
        <div class="w-950px h-950px bg-white p-8 rounded-lg shadow-lg">
            <div class="pl-[360px]">
                <button @click="closePopup">
                    <img src="../svg/CloseCircle.svg" alt="image">
                </button>
            </div>
            <div class="pt-5 pl-11 pr-7">
                <img src="../popup/svg/Group 52.svg" alt="image">
                <p class="text-center pt-7 font-['Montserrat'] text-[20px]">Congratulations, your password has been <br />
                    successfully changed! Don't forget to <br /> remember your new password.</p>
            </div>
        </div>
    </div>
</template>

<script>
export default {
    methods: {
        closePopup() {
            this.$router.push("/login");
        }
    }
}
</script>
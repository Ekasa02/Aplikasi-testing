<template>
  <div class="py-5 px-5 items-center justify-end">
    ...
    <div v-for="item in items" :key="item.id" class="flex py-4">
      <button @click="runGetResultForItems" :class="getButtonClass(item.method) + ' btn-size'" class="font-montserrat text-white font-semibold rounded">
        {{ item.method }}
      </button>
  ...
  methods: {
    ...
    async runGetResultForItems() {
      for (const item of this.items) {
        try {
          const response = await this.$axios.$get(`/automatic/run/${this.verssionId}/${item.id}`);

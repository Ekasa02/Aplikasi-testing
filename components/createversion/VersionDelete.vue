  ...
  >
    <div class="bg-white p-8 rounded-lg shadow-lg">
      ...
      <div class="pt-6 pl-11 pr-7">
        ...
        <p class="text-center pt-7 font-['Montserrat'] text-[20px]">
          Are you sure to delete version
        </p>
        <h1 class="font-bold text-center">{{ versi}}</h1>
      </div>
      <div class="flex pt-[30px] justify-center items-center gap-x-5">
        ...
        <button
          class="font-['Montserrat'] bg-[#554AF0] text-white font-bold py-2 px-4 rounded"
          @click="deleteItem"
        >
          Delete
        </button>
    ...
    async getVersion() {
      try {
        const response = await this.$axios.get(
            `/versions/${this.item}`        )
          this.versi=response.data.data[0].name
        // console.log(response)
      } catch (error) {
        console.log(error)
      }
